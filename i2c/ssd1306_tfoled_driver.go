...
const (
	...
	ssd1306ChargePumpSettingTfoled = 0x8D
)
...
func (i *SSD1306Init) GetSequence() []byte {
	return []byte{
		...
		ssd1306SetSegmentRemap127, ssd1306SetComOutput8,
...
//
func NewSSD1306Driver(a i2c.Connector, options ...func(i2c.Config)) *SSD1306Driver {
	s := &SSD1306Driver{
...
func (s *SSD1306Driver) Start() (err error) {
	...
	s.initSequence = &SSD1306Init{
		displayClock:         0xB1,
		multiplexRatio:       0x1F,
		displayOffset:        0x10,
		startLine:            0x00,
		chargePumpSetting:    0x8B, // 0x10 if external vcc is set
		memoryAddressingMode: 0x00,
		comPins:              0x02,
		contrast:             0x4D, // 0x9F if external vcc is set
		prechargePeriod:      0x62, // 0x22 if external vcc is set
		vComDeselectLevel:    0x3F,
	}
...
func WithSSD1306DisplayWidth(val int) func(i2c.Config) {
...
func WithSSD1306DisplayHeight(val int) func(i2c.Config) {
	return func(c i2c.Config) {
		d, ok := c.(*SSD1306Driver)
		if ok {
			d.displayHeight = val
...
func (s *SSD1306Driver) Display() (err error) {
	pages := s.displayHeight / s.pageSize
	for p := 0; p < pages; p++ {
		pi := int(0xB0) + p
		err = s.commands([]byte{byte(pi), 0x00, 0x10})
		_, err = s.connection.Write(append([]byte{0x40}, s.buffer.buffer[p*128:(p+1)*128]...))
	}
